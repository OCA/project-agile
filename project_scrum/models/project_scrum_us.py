...
class ProjectScrumUs(models.Model):
    ...
    task_test_ids = fields.One2many(
        comodel_name="project.scrum.test", inverse_name="user_story_id_test",
    )
    ...
    company_id = fields.Many2one(
        related="project_id.analytic_account_id.company_id", store=True
    )
    ...
    reference = fields.Char(
        "Number", index=True, readonly=True, copy=False, default="/",
    )
    ...
    def _resolve_project_id_from_context(self):
        ...
        if isinstance(context.get("default_project_id"), basestring):
            ...
            if not project_ids:
                project_ids = project_ids.name_search(project_name, operator="=ilike",)
            if not project_ids:
